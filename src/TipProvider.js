...
export default class TipProvider extends Component {
	...
	animatePulse = () => {
		const { showItemPulseAnimation = this.props.showItemPulseAnimation } = this.state
	...
	renderTip() {
		...
		return (
			...
			>
				...
				<View
					style={{
						...
						top: arrowPosition?.top
							? arrowPosition?.top - (tipStyle.borderWidth + 1 || 0)
							: undefined,
						bottom: arrowPosition?.bottom
							? arrowPosition?.bottom - (tipStyle.borderWidth + 1 || 0)
							: undefined,
				...
				<TouchableOpacity activeOpacity={0.8} disabled={!onTipPress} onPress={onTipPress}>
					{renderTip ? (
						renderTip({
